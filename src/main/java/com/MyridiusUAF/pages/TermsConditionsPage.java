...
public class TermsConditionsPage extends BasePage {
    ...
    private static final Logger logger = LoggerFactory.getLogger(TermsConditionsPage.class);
    ...
    @FindBy(xpath = "//input[@id='customers_map_primary_employment_employer']")
    private WebElement employer;

    @FindBy(xpath = "//input[@id='customers_map_primary_employment_occupation']")
    private WebElement occupation;

    @FindBy(xpath = "//label[@for='customers_map_primary_customProps_currentlyEmployedyes' and contains(normalize-space(.),'Yes')]")
    private WebElement employedYes;

    @FindBy(xpath = "//label[@for='customers_map_primary_customProps_empOrSocialSecurityno' and contains(normalize-space(.),'No')]")
    ...
    @FindBy(xpath = "//div[@class='wdg-label-content ng-binding' and contains(normalize-space(.),'E-Disclosure')]")
    private WebElement encaCheckboxLabel;
    ...
    public void employerDetails() throws InterruptedException
    {
        ...
        waitUntilClickable(employedYes,80);
        click(employedYes,"Employed Selected Yes");
        waitUntilVisible(employer);
    ...
    public void validateName(String expectedName)
    {
        ...
        String fullText = encaCheckboxLabel.getText();
//        logger.info("Logging full checkbox text: " + fullText);
        // Example fullText: "I, eshtra testing only, have reviewed and consent to the E-Disclosure."
        // Extract the name using string manipulation
        String fullNameText = "";
        int start = fullText.indexOf(",") + 2;
        int end = fullText.indexOf(",", start);
        if (start > 0 && end > start) {
            fullNameText = fullText.substring(start, end).trim();
        }
        // Output the name
        logger.info("Logging full name text: " + fullNameText);

        Assert.assertEquals(fullNameText, expectedName, "Name mismatch between pages");
    }
    ...
    public void encaPage() throws InterruptedException
    {
        ...
        String expectedName = TestDataStore.getStartAppFullName();
        String actualName = fullNameText;
        boolean isMatching = actualName.equals(expectedName);

        if (isMatching) {
            logger.info("Name match successful: actualName matches expectedName.");
        } else {
            logger.warn("Name match failed: actualName does not match expectedName.");
        }

        //Assert.assertEquals(fullNameText, startAppFullName, "The displayed value does not match the entered value!");
    }
